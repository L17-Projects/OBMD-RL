...
class SO2ReplayBuffer(ReplayBuffer):
    ...
    def sample(self, batch_size, env=None):
        ...
        obs_rot = rotate_batch_obs(
            obs,
            phi,
            self.N_atoms
        )

        next_obs_rot = rotate_batch_obs(
            next_obs,
            phi,
            self.N_atoms
        )

        actions_rot = rotate_batch_actions(
            actions,
            phi
        )
        ...
        # numpy -> torch
        # -------------------------------------------------
        obs_rot = th.as_tensor(
            obs_rot,
            device=batch.observations.device,
            dtype=batch.observations.dtype
        )

        next_obs_rot = th.as_tensor(
            next_obs_rot,
            device=batch.next_observations.device,
            dtype=batch.next_observations.dtype
        )

        actions_rot = th.as_tensor(
            actions_rot,
        ...
        # -------------------------------------------------
        # concatenate original + rotated
        # -------------------------------------------------
        observations = th.cat(
            [batch.observations, obs_rot],
            dim=0
        )

        actions = th.cat(
            [batch.actions, actions_rot],
            dim=0
        )

        next_observations = th.cat(
            [batch.next_observations, next_obs_rot],
            dim=0
        )
