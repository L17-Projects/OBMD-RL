...
class RigidBodyRotationEnv(gym.Env):
    ...
    def __init__(self, N=4, max_angle=np.pi/18):
        super().__init__()
    ...
    def reset(self, seed=None, options=None):
        ...
        ## najbrz ker hoces nov target + nov initial position za vsako novo epizodo
        ...
        # calc angle difference between noisy and current conf.abs
    ...
    def step(self, action):
        ...
        #reward = -np.sqrt(angle) #np.exp(-5*angle) - 1 #np.cos(angle) - 1 #-np.sqrt(angle) 
        reward = -angle
        #if angle < 10/180*np.pi:
        #    #print('Max reward!!!!')
        ...
        # tukaj bi lahko dal geodezicno razdaljo
        # d = np.arccos((np.trace(R_target.T @ R_current) - 1) / 2)

        terminated = angle < 5/180*np.pi  # 5 degrees
        terminated = False
        if(terminated):
            print(f"Episode terminated: angle threshold reached.")
