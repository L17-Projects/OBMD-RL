...
def evaluate_model(model_name, model_type, N_episodes=200, start_index=0, workdir="lammps"):
    ...
    for k in range(1):#N_episodes):  just one episode for measuring the protocol
        obs, _ = env.reset()
        product_values = []
        # target postions
        ...
        for j in range(30): # modified 9.3.2026: 15->30 steps per episode. 9.3. 15 je dovolj. za vsak slucaj 20
            ...
            action, _states = model.predict(obs, deterministic=False) #True)
            action_dict[j].append(action)

            print('predicted actions are:', action)

            obs, reward, terminated, truncated, info = env.step(action) #action*(-1)) mod. 8.3. ... pomoje ker je bil prej testiran model z rotacijskimi matrikami
    ...
    np.save(
        f"models/action_dict_{model_name}_{start_index}.npy",
        dict(action_dict),
        allow_pickle=True,
    )
